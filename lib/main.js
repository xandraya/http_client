...
class HTTPClient {
    constructor(opt) {
        Object.defineProperty(this, "_opt", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        Object.defineProperty(this, "_pub_sufix", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
        ...
        Object.defineProperty(this, "_secureAgent", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
...
\ \ \ \ \ \ \ ${cookie.host_only_flag}, ${cookie.secure_only_flag}, ${cookie.http_only_flag})`);
    ...
    async createTables() {
        await this._client.query(`create table if not exists cookie_store (name varchar(256), value varchar(4096), creation_time bigint, last_access_time bigint, \
\ \ \ \ expiry_time bigint, domain varchar(256), path varchar(256), host_only_flag boolean, secure_only_flag boolean, http_only_flag boolean)`);
    ...
    async loadPublicSufixes() {
        ...
        if (!result.rows.length || result.rows.length && Date.now() - Number(result.rows[0].timestamp) > 1000 * 60 * 60 * 24) {
            ...
            this._pub_sufix = JSON.parse(result.rows[0].data);
    }
    async loadCookieStore() {
    ...
    validateCookieDate(date) {
        ...
        let parsedDate = { hour: 0, minute: 0, second: 0, day_of_month: 0, month: 'jan', year: 0 };
            ...
            while (true) {
                ...
                matching: {
                    ...
                    if (date_token.toLowerCase().match(/(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)/) && !found_month) {
                        parsedDate.month = date_token.toLowerCase();
                        found_month = true;
                        break matching;
                    }
        ...
        return (new Date(`${parsedDate.day_of_month} ${parsedDate.month} ${parsedDate.year} ${parsedDate.hour}:${parsedDate.minute}:${parsedDate.second} GMT`)).toUTCString();
    }
    computeDefaultPath(url) {
        let uri_path = '';
        if (url)
            uri_path = url.split(String.fromCharCode(0x3F))[0];
        if (!uri_path || !uri_path.startsWith(String.fromCharCode(0x2F)))
            return String.fromCharCode(0x2F);
        if ([...uri_path.matchAll(/\//g)].length === 1)
            return String.fromCharCode(0x2F);
        else
            return uri_path.slice(0, uri_path.lastIndexOf(String.fromCharCode(0x2F)));
    }
    computeCanonHostname(host) {
        return host.toLowerCase();
    }
    domainMatch(hostname, domain) {
        return hostname === domain || hostname.endsWith(domain) && hostname.slice(0, -domain.length).endsWith(String.fromCharCode(0x2E));
    }
    pathMatch(request_path, cookie_path) {
        const pos = request_path.search(/\?/);
        if (pos >= 0)
            request_path = request_path.slice(0, pos);
                ...
                for (let attr of unparsed_attr) {
                    ...
                    if (name.toLowerCase() === 'domain') {
                        ...
                        if (value[0].match(/\x2E/))
                            ...
                            validated.domain = value.toLowerCase();
                        continue;
                    }
    ...
    computeTempStore(req, res) {
        const store = this._store;
        const attrListArray = this.parseSetCookie(res);
        const tempStore = [];
        const canonHostname = this.computeCanonHostname(req.host);
        let domain_attribute;
        for (let attrList of attrListArray) {
            let cookie = { name: attrList.name, value: attrList.value, creation_time: Date.now(), last_access_time: Date.now() };
            if (attrList.max_age || attrList.expires) {
                cookie.persistent_flag = true;
                cookie.expiry_time = attrList.max_age ? (new Date(attrList.max_age)).getTime() : (new Date(attrList.expires)).getTime();
            }
            else {
            ...
            if (dump)
                continue;
            if (cookie.name.startsWith('__Secure-') && !cookie.secure_only_flag)
                continue;
    ...
    async handleHTTPError(res, opts, cb, postData) {
        if ([301, 302, 307, 308].includes(res.statusCode)) {
            ...
            if (!res.headers.location)
                throw { code: res.statusCode, message: 'Redirect path not obtained' };
            res.headers.location.startsWith('http') ?
                regex = /https:\/\/([^\/]*)(\/.*)/.exec(res.headers.location) :
                regex = /(\/.*)/.exec(res.headers.location);
            if (!regex)
                throw { code: res.statusCode, content_type: res.headers['content-type'], message: 'Could not parse URL' };
            regex.shift();
            path = regex.pop();
            if (!path)
                throw { code: res.statusCode, content_type: res.headers['content-type'], message: 'Could not parse PATH' };
            host = regex.pop();
            if (!host)
                host = opts.host;
    ...
    printDebug(req, res) {
        ...
        if (this._opt.debug === 2) {
            ...
            freeSockets && freeSockets.forEach(s => {
                ...
                if (socket)
                    sockets.push(`${socket[0].localAddress}:${socket[0].localPort}`);
            });
            for (let cookie of this._store)
    ...
    async request(opts, cb, postData) {

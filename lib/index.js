...
\ \ \ \ expiry_time bigint, domain varchar(256), path varchar(256), host_only_flag boolean, secure_only_flag boolean, http_only_flag boolean)`);
                ...
                for (let attr of unparsed_attr) {
                    ...
                    if (name.toLowerCase() === 'path') {
                        if (!value || !value[0].match(/\x2F/))
                            ...
                            validated.path = res.url.split(String.fromCharCode(0x3F))[0];
                        continue;
                    }
    ...
    parseCookie(host, path) {
        const store = this._store;
        let parsed_cookie_list = '';
        for (let i = 0; i < store.length; i++) {
            Date.now() - store[i].expiry_time > 0 && store.splice(i, 1);
        }
        ...
        switch (res.statusCode) {
            case 302:
                ...
                if (!res.headers.location)
                    throw new Error('302 - redirect path not obtained');
                regex = /https:\/\/([^\/]*)(.*)/.exec(res.headers.location);
                if (!regex)
                    throw new Error('302 - could not parse URL');
                path = regex.pop();
                if (!path)
                    throw new Error('302 - could not parse PATH');
                host = regex.pop();
                if (!host)
                    throw new Error('302 - could not parse HOST');
